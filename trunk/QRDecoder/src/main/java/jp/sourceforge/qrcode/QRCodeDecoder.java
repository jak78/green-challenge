...
import jp.sourceforge.qrcode.geom.Point; //import jp.sourceforge.qrcode.ecc.ReedSolomon;
...
public class QRCodeDecoder {
	...
	public byte[] decode(QRCodeImage qrCodeImage)
		...
		return ((DecodeResult) results.elementAt(minErrorIndex))
				.getDecodedBytes();
	}
	...
	Point[] getAdjustPoints() {
		...
		// new Point(-2,-2), new Point(1,-1), new Point(-1,0), new
		// Point(-2,-2)};
	...
	DecodeResult decode(QRCodeImage qrCodeImage, Point adjust)
		...
		try {
			if (numTryDecode == 0) {
				...
				qrCodeSymbol = imageReader
						.getQRCodeSymbolWithAdjustedGrid(adjust);
			}
		...
		// now blocks turn to data blocks (corrected and extracted from original
		// blocks)
		blocks = correctDataBlocks(blocks);

		try {
			byte[] decodedByteArray = getDecodedByteArray(blocks, qrCodeSymbol
					.getVersion(), qrCodeSymbol.getNumErrorCollectionCode());
			...
			if (numLongerRSBlocks == 0) { // symbol has only 1 type of RS block
				...
				for (int i = 0; i < numRSBlocks; i++) {
					if (i < numShorterRSBlocks) { // get shorter RS Block(s)
						...
						canvas.println("eccPerRSBlock(shorter)="
								+ eccPerRSBlock);
						RsDecode corrector = new RsDecode(eccPerRSBlock / 2);
						...
						for (int j = 0; j < lengthLongerRSBlock; j++) {
							...
							longerRSBlocks[i - numShorterRSBlocks][j] = blocks[j
									* numRSBlocks + i - mod];
						}
						canvas
								.println("eccPerRSBlock(longer)="
										+ eccPerRSBlock);
						RsDecode corrector = new RsDecode(eccPerRSBlock / 2);
						int ret = corrector.decode(longerRSBlocks[i
								- numShorterRSBlocks]);
						if (ret > 0)
							numSucceededCorrections += ret;
						else if (ret < 0)
				...
				for (int i = 0; i < numRSBlocks; i++) {
					if (i < numShorterRSBlocks) {
						for (int j = 0; j < lengthShorterRSBlock
								- eccPerRSBlock; j++) {
							dataBlocks[p++] = shorterRSBlocks[i][j];
						}
					} else {
						for (int j = 0; j < lengthLongerRSBlock - eccPerRSBlock; j++) {
							dataBlocks[p++] = longerRSBlocks[i
									- numShorterRSBlocks][j];
						}
					}
				}
			}
			if (numSucceededCorrections > 0)
				canvas.println(String.valueOf(numSucceededCorrections)
						+ " data errors corrected successfully.");
			else
	...
	byte[] getDecodedByteArray(int[] blocks, int version,
			int numErrorCorrectionCode) throws InvalidDataBlockException {
		byte[] byteArray;
		QRCodeDataBlockReader reader = new QRCodeDataBlockReader(blocks,
				version, numErrorCorrectionCode);
		try {
